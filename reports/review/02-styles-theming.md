...
> **Batch 3 note (resolved).** `packages/themes/src/themes.spec.ts` is now that static contrast spec: it parses every `themes/*.css` block, resolves `var()` chains through the theme and `base.css` and evaluates `color-mix()` with culori, then asserts per theme — AA 4.5:1 for class-A ink on the four neutral surfaces, class-C intents on body/surface/field **and** on their own `*-subtle` fill, `fg-on-<intent>` on `bg-<intent>`; 3:1 for `border-focus` on every surface and for the new `border-control` token on body/surface/field; and 7:1 (AAA) for class-A ink with the remaining gaps enumerated in `AAA_KNOWN_GAPS` (a listed pair that starts passing fails the test until it is removed, so the list only shrinks). It also pins `color-scheme` per theme, the `:root:not([data-theme])` fallback and the `--kj-transition` aliasing the reduced-motion guard relies on. `reports/a11y/*` stays what it is — a rendered-page axe snapshot — and is no longer the evidence for token contrast.
