 ...
 *
 * Reads from the pre-built `docs-manifest.json` file (generated by `extract-docs.ts`)
 * instead of re-running ts-morph extraction, which avoids tsconfig path issues during
 * Angular's prerender build phase where `process.cwd()` is the `apps/docs` subdirectory.
 */
export function getDocsSlugs(): string[] {
  ...
  for (const p of candidates) {
    try {
      const manifest = JSON.parse(readFileSync(p, 'utf-8')) as DocsManifest;
      return manifest.components.map(c => c.slug);
    } catch {
      // try next candidate
    }
  }
  // Fall back to ts-morph extraction if JSON not found
