...
@Component({
  ...
  imports: [KjButton],
  styles: [`
    :host { display: block; background: var(--kj-surface); border: 1px solid var(--kj-border); border-radius: var(--kj-radius-lg); padding: 1.5rem; min-width: 20rem; color: var(--kj-text); font-family: var(--kj-font); box-shadow: var(--kj-shadow-hard); }
    h2 { margin: 0 0 0.5rem; font-size: 1.125rem; color: var(--kj-text); }
    ...
    .dialog-actions button[kjButton] { padding: 0.4rem 1rem; font-size: 0.8125rem; }
...
@Component({
  ...
  `],
  template: `
    <button kjButton [kjVariant]="'default'" (click)="open()">Open Dialog</button>
  `,
})
export class DialogBasicExample {
  private readonly dialog = inject(KjDialogService);
  open() { this.dialog.open(DialogBasicContent); }
}
