...
const template = `<div kjSelect [(kjSelectValue)]="val"><button kjSelectTrigger aria-haspopup="listbox">Select</button><div kjSelectContent role="listbox"><div kjOption [kjOptionValue]="'apple'" role="option">Apple</div><div kjOption [kjOptionValue]="'banana'" role="option">Banana</div></div></div>`;
...
describe('KjSelectDirective', () => {
  ...
  it('clicking option selects and closes', async () => {
    ...
    expect(container.querySelector('[kjOption]')).toHaveAttribute('aria-selected', 'true');
    expect(container.querySelector('[kjSelectContent]')).toHaveAttribute('hidden', '');
  });
